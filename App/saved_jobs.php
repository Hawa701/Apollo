...
$offset = ($currentPage - 1) * $jobsPerPage; //the LIMIT offset needed for the MySQL query based on the current page number
...
function addJob($connect, $jobId) {
    $query = "SELECT job.Job_Title, job.Description, job.Proposals, job.Payment, job.Experience, job.Date, job.Token, job.Estimated_Time FROM job 
                  JOIN saved_jobs ON job.Job_ID = saved_jobs.Job_ID 
                  WHERE saved_jobs.Job_ID = $jobId";
                       $result = mysqli_query($connect, $query);
                       ...
                       if($result && mysqli_num_rows($result) > 0) {
                        ...
                        <div class='job'>
                                ...
                                <div class='job-title'>
                                    <h3 onclick='openApplyJob()'>{$row['Job_Title']}</h3>
                                ...
                                <div class='time'>
                                    <div class='proposal'>Proposal: <span>Less than <span>{$row['Proposals']}</span></span></div>
                                    <div class='tokens'>Tokens To Apply: <span>{$row['Token']}</span></div>
                                </div>
...
<html lang="en">
  <head>
    ...
    <!-- css link -->
    <link rel="stylesheet" href="./Style/saved_jobs.css?v=1.0" />
    <!-- icon link -->
    <link
      rel="stylesheet"
